...
const extensionNameLong = "SillyBunny-GroupUtilities";
...
jQuery(async () => {
	...
	const settingsHtml = $(await $.get(`${extensionFolderPath}/html/presence-settings.html`));
	...
	$("#extensions_settings #presence_settings").remove();
	$("#extensions_settings").append(settingsHtml);
    $("#presence_disableTransition").trigger('change');

    const universalTrackerAlwaysOn = `
        <label class="checkbox_label whitespacenowrap sbu-presence-universal-tracker" title="Set the universal tracker to active for new messages" style="margin-top: 7px">
            <input id="presence_universal_tracer_on" type="checkbox"/>
            <span data-i18n="Universal Tracker">Universal Tracker</span>
        </label>
    `;

    $('#GroupFavDelOkBack .sbu-presence-universal-tracker').remove();
